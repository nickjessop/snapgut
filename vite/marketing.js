 ...
 *      `dist/index.html`, … — which is where the Origin_Server looks for it. The
 *      same step then walks every emitted document and fails the build if it
 *      references an asset that is not in the Build_Output (Requirement 7.4).
 *
 ...
 *      same step writes:
 *
 *        dist/robots.txt        restrictive (Disallow: /) by default; allows
 *                               Marketing_Pages only when PUBLIC_ORIGIN is set
 *        dist/sitemap.xml       empty by default; lists indexable paths as
 *                               absolute URLs only when PUBLIC_ORIGIN is set
 *
...
/**
 * `robots.txt`: restrictive by default (Disallow: /) unless a PUBLIC_ORIGIN is
 * provided, in which case it allows the indexable paths and references the sitemap.
 *
 * @param {{ origin?: string|null, paths?: string[] }} [options]
 * @returns {string}
 */
export function robotsTxt({ origin = null, paths = indexablePaths() } = {}) {
  if (!origin) {
    return [
      "# Generated by vite/marketing.js — no PUBLIC_ORIGIN set, restrictive by default.",
      "User-agent: *",
      "Disallow: /",
      "",
    ].join("\n");
...
/**
 * `sitemap.xml`: empty unless a PUBLIC_ORIGIN is provided. When set, lists
 * exactly the indexable paths as absolute URLs.
 *
 ...
 * @returns {string}
 */
export function sitemapXml({ origin = null, paths = indexablePaths() } = {}) {
  if (!origin) {
    return [
...
/**
 * Write the generated artifacts: robots.txt and sitemap.xml.
 * Both are restrictive by default (no PUBLIC_ORIGIN at build time).
 *
 * @param {string} outDir absolute path of the Build_Output root
 */
